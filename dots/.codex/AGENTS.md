# Global Personal Coding-Agent Instructions
...
### Tooling preferences
- **Prefer CLI over MCP** when both provide equivalent access and policy
  permits either one. Fall back to MCP when no CLI covers the task.
...
- **Before committing, run the project's linter on every changed file it
  ...
  automated code review, or manual inspection is not a substitute.
...
- **Run `git diff` in a separate shell invocation.** A failure from an
  unrelated command can mask or truncate the diff. A zero-match `ug`
  exits 1, which is enough to do it in some environments. If output was
  hidden, I have not seen the diff -- paste the hunks into the reply.
...
- Use the environment's PR-feedback workflow when available. Otherwise
  follow this ordering: change, commit, push, *then* reply. Never tell a
  reviewer feedback is addressed before the commit has landed on the
  remote.
...
- **Attribution**: sign off `-- {agent} & {user}` on any such text the
  agent drafted. Replace `{agent}` with the active agent's public product
  name and `{user}` with my configured public attribution label. Use a
  label only when it is explicit in the conversation or existing
  artifact; otherwise ask. Never read an environment variable to
  discover it. Solo credit only when I drafted it and the agent merely
  edited.
...
- Use the shell identified by the active environment. Do not infer the
  shell language from the tool's name.
- When the shell is zsh, do not use Bash-only constructs such as
  `declare -A` or `${!arr[@]}`. zsh does not word-split unquoted
  variables: `for x in $var` iterates once over the whole string. For
  list iteration use `printf '%s\n' a b c | while read x; do ...;
  done`. Verify a single-item case before looping over many.
- Treat shell invocations as isolated unless the environment explicitly
  guarantees state persistence. Supply anything a command needs in the
  same invocation.
- Use the tool's supported timeout or resumable-session mechanism. Block
  on the real artifact instead of sleeping past the tool's observation
  window. Confirm which file receives the output -- a wrapper's log is
  not the redirect target. If an attempt yields no new information,
  change the approach.
- When zsh has `noclobber` enabled, use `>|` when intentionally
  overwriting an existing file. A bare `>` can fail while the next
  command still runs against the stale file, making a re-run appear to
  prove two things identical when they are not.

### Ruby runtime activation

Before running Ruby, Bundler, Rails, RSpec, RuboCop, or another
Ruby-backed command in a project:

- Read the project-root `.ruby-version`. If its value starts with
  neither `ruby` nor `jruby`, prefix it with `ruby-`; for example,
  `3.4.8` maps to `ruby-3.4.8`.
- Resolve the runtime directory as
  `$HOME/.rubies/<normalized-version>/bin`. Fail visibly if that
  directory does not exist; do not silently fall through to another
  installed Ruby.
- Preserve the existing executable search path, remove prior
  `$HOME/.rubies/*/bin` entries from it, and prepend the resolved runtime
  directory. In Fish, the user's `setruby` function performs this
  activation; from another shell, reproduce its semantics explicitly.
- Before relying on a verification result, confirm `command -v ruby`,
  `ruby --version`, `command -v bundle`, and `bundle --version` all
  resolve through the selected runtime.
- Treat a Ruby/Bundler version mismatch as an environment problem to
  surface and correct immediately. Retry the intended command after
  activation before reporting it as an unverified check.

Reference implementation:
`/Users/jamesbunch/git/bootstrap/dots/.config/fish/functions/setruby.fish`.
