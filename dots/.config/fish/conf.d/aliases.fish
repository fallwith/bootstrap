alias 15=fifteen
alias b='bundle exec'
alias cp='cp -i'
alias brake='bundle exec rake'
alias brubo='bundle exec rubocop'
alias bspec='bundle exec rspec'
alias cdgem=gemcd
alias defaultgems='cat $HOME/.default-gems | grep -v "^#" | xargs -n 1 gem install'
alias dirs='fd -td'
alias dockerstop='docker ps -aq |xargs docker container stop'
alias fd='fd --color never'
alias funcs='vi ~/.config/fish/functions'
alias ghostty=/Applications/Ghostty.app/Contents/MacOS/ghostty
alias ghosttyconfig='nvim_launch ~/.config/ghostty/config'
alias guide='nvim_launch ~/git/public/vim_guide/vim_guide.md'
alias ll='eza --long --all --group --numeric --classify --git --time-style long-iso'
alias lle='ll --extended'
alias mv='mv -i'
alias nvim='nvim_launch'
alias plugins='nvim_launch ~/.config/nvim/init.lua'
alias railsc='bundle exec bin/rails c'
alias railss='bundle exec bin/rails s'
alias running='ps auwx|grep -E "memcache|mongo|mysql|rabbit|redis|postgres|ruby|rails|puma|node|\.rb|git|gradle|rubocop"|grep -v grep'
alias ug='ug --fixed-strings'
alias v='nvim_launch -c "lua require(\'snacks\').picker.smart()"'
alias vi='nvim_launch'
alias vim='nvim_launch'
alias wiki="nvim_launch -c \"lua require('kiwi').open_wiki_index()\""
alias wt='worktree'

abbr -a ayu "fish_config theme save 'ayu Mirage'"
abbr -a brewinstall '/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"'
abbr -a brewtaps 'brew list --full-name | grep /'
abbr -a brewupdate 'brew update && brew bundle --global --verbose && brew upgrade && brew upgrade --cask (brew outdated --cask) && brew cleanup'
abbr -a colimastart 'colima start --vm-type vz --vz-rosetta --arch aarch64 --cpu 8 --memory 8 --disk 64'
abbr -a config 'cd ~/.config/fish'
abbr -a font 'kitty @ set-font-size'
abbr -a gpgtest 'echo testing | gpg --clearsign'
abbr -a gom "gometalinter --enable-all --line-length=120 --deadline=180s ./..."
abbr -a got 'go test -v ./...'
abbr -a matrix 'cxxmatrix -s rain-forever --preserve-background --no-diffuse --rain-density 2.0 --synchronized-update --frame-rate=60'
abbr -a pull 'git pull'
abbr -a push 'git bpush'
abbr -a rebase 'git pull --rebase'
abbr -a servicesstart 'brew services --all start'
abbr -a servicesstop 'brew services --all stop'
abbr -a uga 'ug --all'
abbr -a uge 'ug --perl-regexp'
abbr -a which 'fishwhich'
abbr -a xattrdel 'xattr -c -r'
