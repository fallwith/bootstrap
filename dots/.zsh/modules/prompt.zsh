...
# PROMPT='%{$blue039%}%c%{$reset_color%} %{$grey242%}%T%{$reset_color%}$(git_branch) %{$violet013%}${VIMODE} %{$reset_color%}${emkoala}  %{$bg[white]%}$emmoto% {$reset_color%} ${ret_status}%{$green112%}%p%{$reset_color%} '
